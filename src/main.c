...
static void show_simple_window(bool* showDemoWindow, bool* showAnotherWindow, ImVec4* clearColor);
static void show_node_editor_window();
...
int main(int argc, char* argv[])
{
	...
	bool showDemoWindow, showAnotherWindow;
	showDemoWindow = true;
	...
	while (!quit)
	{
		...
		show_simple_window(&showDemoWindow, &showAnotherWindow, &clearColor);
		show_node_editor_window();
...
static void open_repo(char* repo_path) {
	git_repository* repo;
	int error;
	git_libgit2_init();
	error = git_repository_open(&repo, repo_path);
